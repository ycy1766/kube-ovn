...
func (c *Controller) formatSubnet(subnet *kubeovnv1.Subnet) (*kubeovnv1.Subnet, error) {
	...
	if !newSubnet.Spec.U2OInterconnection || newSubnet.Spec.Vlan == "" {
		newSubnet.Spec.U2OFeatures = kubeovnv1.U2OFeatures{}
	}
...
func (c *Controller) addPolicyRouteForU2OInterconn(subnet *kubeovnv1.Subnet) error {
	...
	if err := c.deletePolicyRouteForU2ODisableInternalUnderlayDirectRouting(subnet); err != nil {
		return err
	}
	if u2oDisableInternalUnderlayDirectRoutingEnabled(subnet) {
		klog.Infof("add u2o internal underlay direct routing policies for subnet %s with shared address sets %s and %s", subnet.Name, disabledInternalDirectRouteCIDRs4Ag, disabledInternalDirectRouteCIDRs6Ag)
	}
	...
	for cidrBlock := range strings.SplitSeq(subnet.Spec.CIDRBlock, ",") {
		...
		match4 := fmt.Sprintf("%s.src == %s && %s.dst == $%s && %s.dst != %s", ipSuffix, cidrBlock, ipSuffix, disabledInternalDirectRouteCIDRsAg, ipSuffix, cidrBlock)
		...
		/*
			...
			priority 29450 match: "ip4.src == underlay subnet cidr &&
			                       ip4.dst == disabled internal direct route cidrs &&
			                       ip4.dst != underlay subnet cidr"                         action: reroute physical gw
			...
			policy4 priority is lower than policy2, but higher than policy1 and policy3. The source match keeps
			overlay-to-underlay traffic on policy1, while underlay-to-underlay traffic overrides the policy3 fallback.
		*/
		action := kubeovnv1.PolicyRouteActionAllow
		if subnet.Spec.Vpc == c.config.ClusterRouter {
		...
		if u2oDisableInternalUnderlayDirectRoutingEnabled(subnet) && len(disabledInternalDirectRouteCIDRs) > 0 {
			...
			if err := c.addPolicyRouteToVpc(
				...
				buildPolicyRouteExternalIDs(subnet.Name, map[string]string{
...
func (c *Controller) deletePolicyRouteForU2ODisableInternalUnderlayDirectRouting(subnet *kubeovnv1.Subnet) error {
	lr := subnet.Status.U2OInterconnectionVPC
	if lr == "" {
		lr = subnet.Spec.Vpc
	}
	...
	if c.logicalRouterExists(lr) {
		policies, err := c.OVNNbClient.ListLogicalRouterPolicies(lr, util.U2ODisableInternalUnderlayDirectRoutingPolicyPriority, map[string]string{
			"isU2ODisableInternalUnderlayDirectRoutePolicy": "true",
			"vendor": util.CniTypeName,
			"subnet": subnet.Name,
		}, true)
